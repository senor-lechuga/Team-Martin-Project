...
public class DisplayCalendar extends JFrame{
    ...
    for(int j=1;j<25;j++){
      for(int i=0;i<5;i++){
        String time;
         ...
         appButtons[j][i].setBackground(Color.LIGHT_GRAY);
  ...
  private String[][] getWeeklyApps (java.util.Date date, String s) throws SQLException{
    ...
    for(int i=0; i<5;i++){
      java.sql.Date tempDate = new java.sql.Date(c.getTime().getTime());
      java.sql.Date sqlDate = new java.sql.Date(tempDate.getYear()+1900,tempDate.getMonth()+1,tempDate.getDate());
      ...
      for(int j=0;j<5;j++){
        if(p=="both")
          ...
          appointments[i][j]= "No "+p+" appointments for this slot!";
	        appButtons[i+1][j].setBackground(Color.LIGHT_GRAY);
          takenSlot[i][j] = false;
      }
    }
  }
  ...
  private ActionListener dentistListener = new ActionListener(){
      ...
      try{
        reset("Dentist");
        appointments = getWeeklyApps(mon,"Dentist");
      } catch(SQLException s){
        s.printStackTrace();
      }
		}
  ...
  private ActionListener hygienistListener = new ActionListener(){
    public void actionPerformed(ActionEvent e){
      try{
        reset("Hygienist");
        appointments = getWeeklyApps(mon,"Hygienist");
      } catch(SQLException s){
        s.printStackTrace();
      }
    }
